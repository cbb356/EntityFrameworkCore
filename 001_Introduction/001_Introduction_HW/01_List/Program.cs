...
{
    /*
    * Розробіть консольну програму з використанням одного списку з типом (List<Order>) 
    * Замовлення – Order
    * 1) + Id: Guid
    * 2) + Name: string
    * 3) + Create: datetime
    ...
    */
    ...
    internal class Program
    {
        ...
        {
            // Create and fill List of Orders
            ...
            //Show orders by index in the list
            ShowByIndex(orders, 1);
            ShowByIndex(orders, 5);
            ShowByIndex(orders, 0);
            ShowByIndex(orders, 7);
            ShowByIndex(orders, 12);

            //Show orders by id
            ShowIndexById(orders, "7B0301AF-02E5-43B3-B94C-A7E4F5418BE3");
            ShowIndexById(orders, "8999AD9D-5CA8-47E1-A14D-16D397AE0BF8");
            ShowIndexById(orders, "00000000-0000-0000-0000-000000000000");
            ShowIndexById(orders, "NonGuid");

            //Show orders by name
            ShowIndexByName(orders, "Order01");
            ShowIndexByName(orders, "Order08");
            ShowIndexByName(orders, "NonExistentOrder");

            // Delay.
            Console.ReadKey();
        }

        public static void ShowByIndex(List<Order> list, int index)
        {
            try
            {
                ...
                if (order != null)
                {
                    Console.WriteLine($"The order with index {index}:\n{order.ToString()}");
                }
                else
                {
                    Console.WriteLine($"The order with index {index} was not found\n");
                }
            }
            catch (Exception ex) 
            {
                Console.WriteLine($"Error finding order with index{index}: {ex.Message}\n");
            }
        }

        public static void ShowIndexById(List<Order> list, string id)
        {
            try
            {
                int index = list.FindIndex(x => x.Id == Guid.Parse(id));
                if (index != -1)
                {
                    Console.WriteLine($"The index of order with Id {id} is: {index}\n");
                }
                else
                {
                    Console.WriteLine($"The order with Id {id} was not found, index is -1\n");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error finding index for Id {id}: {ex.Message}\n");
            }
        }
        public static void ShowIndexByName(List<Order> list, string name)
        {
            try
            {
                ...
                if (index != -1)
                {
                    Console.WriteLine($"The index of order with Name {name} is: {index}\n");
                }
                else
                {
                    Console.WriteLine($"The order with Name {name} was not found, index is -1\n");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error finding index for Name {name}: {ex.Message}\n");
            }
        }
    }
}
